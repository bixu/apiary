...
impl RecipientCommands {
    pub async fn execute(&self, client: &HoneycombClient) -> Result<()> {
        match self {
            RecipientCommands::List { format } => {
                list_recipients(client, format).await
            }
            RecipientCommands::Get { id, format } => {
                get_recipient(client, id, format).await
            }
            ...
            RecipientCommands::Delete { id } => {
                delete_recipient(client, id).await
            }
        }
    }
}
    ...
    match format {
        ...
        OutputFormat::Table => {
            if let Value::Array(recipients) = response {
                println!("{:<15} {:<30} {:<15} {:<30} {}", "ID", "Name", "Type", "Target", "Created");
                println!("{:-<95}", "");
...
async fn create_recipient(client: &HoneycombClient, data: &str, format: &OutputFormat) -> Result<()> {
...
async fn update_recipient(client: &HoneycombClient, id: &str, data: &str, format: &OutputFormat) -> Result<()> {
    let json_data = if std::path::Path::new(data).exists() {
        read_json_file(data)?
    } else {
        serde_json::from_str(data)?
