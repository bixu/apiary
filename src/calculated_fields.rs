...
impl CalculatedFieldCommands {
    pub async fn execute(&self, client: &HoneycombClient) -> Result<()> {
        match self {
            ...
            CalculatedFieldCommands::Get { dataset, id, format } => {
                get_calculated_field(client, dataset, id, format).await
            }
            CalculatedFieldCommands::Create { dataset, data, format } => {
                create_calculated_field(client, dataset, data, format).await
            }
            CalculatedFieldCommands::Update { dataset, id, data, format } => {
...
async fn list_calculated_fields(client: &HoneycombClient, dataset: &str, format: &OutputFormat) -> Result<()> {
    ...
    match format {
        ...
        OutputFormat::Table => {
            if let Value::Array(fields) = response {
                println!("{:<15} {:<30} {:<50} {}", "ID", "Alias", "Expression", "Created");
                println!("{:-<110}", "");
...
async fn get_calculated_field(client: &HoneycombClient, dataset: &str, id: &str, format: &OutputFormat) -> Result<()> {
...
async fn create_calculated_field(client: &HoneycombClient, dataset: &str, data: &str, format: &OutputFormat) -> Result<()> {
...
async fn update_calculated_field(client: &HoneycombClient, dataset: &str, id: &str, data: &str, format: &OutputFormat) -> Result<()> {
    let json_data = if std::path::Path::new(data).exists() {
        read_json_file(data)?
    } else {
        serde_json::from_str(data)?
...
async fn delete_calculated_field(client: &HoneycombClient, dataset: &str, id: &str) -> Result<()> {
    ...
    println!("Calculated field '{}' in dataset '{}' deleted successfully", id, dataset);
